...
void app_time_init()
{
  ...
  {
    ESP_LOGW(APP_TIME_TAG, "Time is not set yet");
    ESP_LOGD(APP_TIME_TAG, "Getting time over NTP");
    /* Initialize sntp */
    ...
    if(SNTP_SYNC_STATUS_COMPLETED == eStatus)
    {
      ...
      ESP_LOGD(APP_TIME_TAG, "The current date/time in UTC is: %s", tcTimeFormatBuff);
